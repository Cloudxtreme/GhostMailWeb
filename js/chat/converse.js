...
// Slightly modified to allow for KRYPTOS client-to-client encryption.
...
//                    console.log('NADA!');
        ...
        this.updateMsgCounter = function () {
            if (this.msg_counter > 0) {
                if (document.title.search(/^Messages \(\d+\) /) == -1) {
                    document.title = "Chat (" + this.msg_counter + ") " + document.title;
                } else {
                    document.title = document.title.replace(/^Messages \(\d+\) /, "Chat (" + this.msg_counter + ") ");
                }
                window.blur();
                window.focus();
        ...
        this.ChatBoxView = Backbone.View.extend({
                ...
                this.$el.attr('id', this.model.get('box_id'))
                    .html(converse.templates.chatbox(
                            _.extend(this.model.toJSON(), {
                                    ...
                                    label_personal_message: __('Encrypted Chat')
                                }
                            )
                        )
                    );
            ...
            clearChatRoomMessages: function (ev) {
                ...
                var result = confirm(__("Are you sure you want to clear the messages from this Group?"));
                if (result === true) {
                    this.$content.empty();
                }
                return this;
...
//                        this.showStatusNotification(message.get('fullname')+' '+__('has stopped typing'));
            ...
            sendMessage: function (text, cipher) {
            ...
            onMessageSubmitted: function (text, obj) {
                ...
                if (ev.keyCode == KEY.ENTER) {
                    ...
                    if (message !== '') {
                        if (this.model.get('chatroom')) {
                            this.onChatRoomMessageSubmitted(message, this);
                        } else {
                            this.onMessageSubmitted(message, this);
                        }
// Krypto will handel the sending
//                        converse.emit('messageSend', message);
                         //converse.emit('messageSend', {message: message, object: this});
                    }
            ...
            insertEmoticon: function (ev) {
                ev.stopPropagation();
                ev.preventDefault();    // silly emoticon Click Back Bug
            ...
            onChatStatusChanged: function (item) {
                ...
                if (this.$el.is(':visible')) {
                    if (chat_status === 'offline') {
                        this.showStatusNotification(fullname.replace(/@.*$/,'')+' '+__('has gone offline'));
                    } else if (chat_status === 'away') {
                        this.showStatusNotification(fullname.replace(/@.*$/,'')+' '+__('has gone away'));
                    } else if ((chat_status === 'dnd')) {
                        this.showStatusNotification(fullname.replace(/@.*$/,'')+' '+__('is busy'));
                ...
                this.$parent.append(
                    this.$el.html(
                        converse.templates.room_panel({
                            ...
                            'label_room_name': __('Group name'),
                            'label_nickname': __('Nickname'),
                            'label_server': __('Server'),
                            'label_join': __('Create Group '),
                            'label_show_rooms': __('Show(-)')
                        })
                    ).hide());
                this.$tabs = this.$parent.parent().find('#controlbox-tabs');
                this.$tabs.append(converse.templates.chatrooms_tab({label_rooms: __('Groups')}));
                return this;
            },
...
//                $available_chatrooms.html('<dt>'+__('No rooms on %1$s',this.model.get('muc_domain'))+'</dt>');
                $available_chatrooms.html('<dt>'+__('No Groups ')+'</dt>');
...
//                    $available_chatrooms.html('<dt>'+__('Groups on %1$s',this.model.get('muc_domain'))+'</dt>');
                    $available_chatrooms.html('<dt>'+__('Groups ')+'</dt>');
                    ...
                    for (i=0; i<this.rooms.length; i++) {
                        ...
                        fragment.appendChild($(
                            converse.templates.room_item({
                                'name':name,
                                'jid':jid,
                                'open_title': __('Click to open this Group'),
                                'info_title': __('Show more information on this Group')
                                })
                            )[0]);
                    }
                        ...
                        function (stanza) {
                            ...
                            $dd.find('span.spinner').replaceWith(
                                converse.templates.room_description({
                                    ...
                                    'label_open_room': __('Open Group'),
                                    'label_permanent_room': __('Permanent Group'),
                                    'label_public': __('Public'),
                                    'label_semi_anon':  _('Semi-anonymous'),
                                    'label_temp_room':  _('Temporary Group'),
            ...
            createChatRoom: function (ev) {
                ev.preventDefault();
                var isNew = false;
                var $name = ev.target.firstChild.data;
                var name, $name,
                    server, $server,
                    jid,
                    nick = this.model.attributes.nick,    
                    chatroom;
//                if (!nick) { 
//                    $nick.addClass('error'); 
//                } else { 
...
//                }
                if (ev.type === 'click') {
                    ...
                    name = ev.target.firstChild.data;
                    isNew=false;
                } else {
                    isNew = true;
...
//                        jid = Strophe.escapeNode(name) + '@' + server;
                        jid = KRYPTOS.utils.ab2hex(KRYPTOS.randomValue(8)) + '@' + server;
...
//                    'nick': nick,
                ...
                if (isNew) {
                    converse.connection.sendIQ(
                            ...
                            function(stanza) {
                                var configArray = [];                
                                configArray.push($(templates.field({ name: 'FORM_TYPE', value: 'http://jabber.org/protocol/muc#roomconfig'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_roomname', value: name}))[0]);                       // Natural-Language Room Name
                                configArray.push($(templates.field({ name: 'muc#roomconfig_persistentroom', value: '0'}))[0]);                  // Whether to Make Room Persistent
                                configArray.push($(templates.field({ name: 'muc#roomconfig_publicroom', value: '0'}))[0]);                      // Whether to Allow Public Searching for Room
                                configArray.push($(templates.field({ name: 'muc#roomconfig_roomdesc', value: name}))[0]);                       // Room Description
                                configArray.push($(templates.field({ name: 'public_list', value: '0'}))[0]);                                    // ?? Not listed on xep-0045
                                configArray.push($(templates.field({ name: 'muc#roomconfig_maxusers', value: '50'}))[0]);                       // Maximum Number of Room Occupants
                                configArray.push($(templates.field({ name: 'muc#roomconfig_whois', value: 'moderators'}))[0]);                  // Affiliations that May Discover Real JIDs of Occupants
                                configArray.push($(templates.field({ name: 'muc#roomconfig_membersonly', value: '1'}))[0]);                     // Whether to Make Room Members-Only
                                configArray.push($(templates.field({ name: 'muc#roomconfig_moderatedroom', value: '0'}))[0]);                   // Whether to Make Room Moderated Only "voiced" members can chat
                                configArray.push($(templates.field({ name: 'members_by_default', value: '0'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_changesubject', value: '0'}))[0]);
                                configArray.push($(templates.field({ name: 'allow_private_messages', value: '0'}))[0]);                
                                configArray.push($(templates.field({ name: 'allow_private_messages_from_visitors', value: 'nobody'}))[0]);
                                configArray.push($(templates.field({ name: 'allow_query_users', value: '0'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_allowinvites', value: '0'}))[0]);                
                                configArray.push($(templates.field({ name: 'muc#roomconfig_allowvisitorstatus', value: '0'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_allowvisitornickchange', value: '0'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_allowvoicerequests', value: '0'}))[0]);                
                                configArray.push($(templates.field({ name: 'muc#roomconfig_voicerequestmininterval', value: '1800'}))[0]);
                                configArray.push($(templates.field({ name: 'muc#roomconfig_enablelogging', value: '0'}))[0]);
                                

                                var iq = $iq({to: jid, type: "set"})
                                    .c("query", {xmlns: Strophe.NS.MUC_OWNER})
                                    .c("x", {xmlns: Strophe.NS.XFORM, type: "submit"});
                                _.each(configArray, function (node) { iq.cnode(node).up(); });
                                return converse.connection.sendIQ(iq.tree(), 
                                function() {
                                        // console.log(' Group Modify success');
                                    },
                                function(err) { 
                                       // console.log(' Group Modify error');
                                       // console.log(err);
                                    }
                                );                              
                            },
        ...
        this.ControlBoxView = converse.ChatBoxView.extend({
            ...
            initialize: function () {
                ...
                if (this.model.get('closed') === false) {
                    this.show();
                } else {
                    this.hide();
                }
                ...
                var $new = converse.templates.occupant(
                    _.extend(
                        this.model.toJSON(), {
                            ...
                            'desc_participant': __('This user can send messages in this Group'),
                            'desc_visitor': __('This user can NOT send messages in this Group')
                    })
                );
                ...
                $el.typeahead({
                    ...
                    source: function (q, cb) {
                        ...
                        _.each(converse.roster.filter(contains(['fullname', 'jid'], q)), function (n) {
                            results.push({value: n.get('fullname').replace(/@.*$/,''), jid: n.get('jid')});
                        });
                        cb(results);
                    },
                    templates: {
...
//                    var reason = prompt(
//                        __(___('You are about to invite %1$s to the chat Group "%2$s". '), suggestion.value, this.model.get('id')) +
//                        __("You may optionally include a message, explaining the reason for the invitation.")
//                    );
                    var reason = this.chatroomview.model.attributes.name;
                    var name = this.chatroomview.model.attributes.name;
                    if (reason !== null) {
                        this.chatroomview.directInvite(suggestion.jid, reason, name);
                    }
        ...
        this.ChatRoomView = converse.ChatBoxView.extend({
                    ...
                    this.$('.chat-body').empty()
                        .append(
                            converse.templates.chatarea({
                                'show_toolbar': converse.show_toolbar,
                                'label_message': __('Encrypted Message')
                            }))
            ...
            toggleOccupants: function (ev) {
                ...
                if (!this.model.get('hidden_occupants')) {
                    ...
                    this.$('div.participants').show().animate({width: '100px'}, this.scrollDown.bind(this));

                }
            },

            directInvite: function (receiver, reason, name) {
                var attrs = {
                ...
                // affiliate member to group
                var room = this.model.get('jid');
                var jid = receiver;
                this.setAffiliation(room, jid, 'member', name, 
                function() {
            ...
            onConsoleReport: function (stanza) {
                console.log(stanza);
            },

            sendChatRoomMessage: function (text, cipher) {
            ...
            onChatRoomMessageSubmitted: function (text, obj) {
                ...
                switch (match[1]) {
                    ...
                    case 'help':
                        this.showHelpMessages([
                            ...
                            '<strong>/ban</strong>: '   +__('Ban user from Group'),
                            '<strong>/clear</strong>: ' +__('Remove messages'),
                            '<strong>/deop</strong>: '  +__('Change user role to participant'),
                            '<strong>/help</strong>: '  +__('Show this menu'),
                            '<strong>/kick</strong>: '  +__('Kick user from Group'),
                            '<strong>/me</strong>: '    +__('Write in 3rd person'),
                            '<strong>/member</strong>: '+__('Grant membership to a user'),
                            '<strong>/mute</strong>: '  +__("Remove user's ability to post messages"),
                            '<strong>/nick</strong>: '  +__('Change your nickname'),
                            '<strong>/op</strong>: '    +__('Grant moderator role to user'),
                            '<strong>/owner</strong>: ' +__('Grant ownership of this Group'),
                            '<strong>/revoke</strong>: '+__("Revoke user's membership"),
                            '<strong>/topic</strong>: ' +__('Set Group topic'),
                    ...
                    case 'member': 
                        this.setAffiliation(
                                ...
                                this.onConsoleReport.bind(this), this.onCommandError.bind(this));
                    ...
                    case 'voice':
                        ...
                        break;
                    default:
//                        this.sendChatRoomMessage(text, obj);
                        converse.emit('messageSend', {message: text, object: obj});
                    break;
                }
            },
                ...
                $inputs.each(function () {
                    ...
                    if (!--count) {
                        that.sendConfiguration(
                            configArray,
                            that.onConfigSaved(that),
                            that.onErrorConfigSaved(that)
                        );
                    }
                });
            ...
            renderPasswordForm: function () {
                ...
                this.$('.chat-body').append(
                    converse.templates.chatroom_password_form({
                        heading: __('This Group requires a password'),
             ...
             */
            infoMessages: {
                100: __('This Group is not anonymous'),
                102: __('This Group now shows unavailable members'),
                103: __('This Group does not show unavailable members'),
                104: __('Non-privacy-related Group configuration has changed'),
                170: __('Group logging is now enabled'),
                171: __('Group logging is now disabled'),
                172: __('This Group is now non-anonymous'),
                173: __('This Group is now semi-anonymous'),
                174: __('This Group is now fully-anonymous'),
                201: __('A new Group has been created')
            },

            disconnectMessages: {
                301: __('You have been banned from this Group'),
                307: __('You have been kicked from this Group'),
                321: __("You have been removed from this Group because of an affiliation change"),
                322: __("You have been removed from this Group because the Group has changed to members-only and you're not a member"),
                332: __("You have been removed from this Group because the MUC (Multi-user chat) service is being shut down.")
            },
            ...
            showErrorMessage: function ($error) {
                ...
                if ($error.attr('type') == 'auth') {
                    if ($error.find('not-authorized').length) {
                        ...
                        this.showDisconnectMessage(__('You are not on the member list of this Group'));
                    } else if ($error.find('forbidden').length) {
                        this.showDisconnectMessage(__('You have been banned from this Group'));
                    }
                    ...
                    if ($error.find('not-allowed').length) {
                        this.showDisconnectMessage(__('You are not allowed to create new Groups'));
                    } else if ($error.find('not-acceptable').length) {
                        this.showDisconnectMessage(__("Your nickname doesn't conform to this Group's policies"));
                        ...
                        this.showDisconnectMessage(__("This Group does not (yet) exist"));
                    } else if ($error.find('service-unavailable').length) {
                        this.showDisconnectMessage(__("This Group has reached it's maximum number of occupants"));
                    }
                }
            },
...
//                if (body.length < 1024) { // if (encrypted)
...
//                  converse.emit('message', {message: $message, object: this, from: Strophe.getBareJidFromJid($message.attr('from'))});
                    converse.emit('message', {message: $message, object: this});
...
//                 console.log(contact.get('fullname').replace(/@.*$/,''));
                if (name) {
                    ...
                    bootbox.confirm(
                    {
                        message: contact.get('fullname').replace(/@.*$/,'') + " " + "has invited you to join a Group chat: " + name,
...
// ------------------- 
// For Krypto
                converse.emit('message', {message: $message, object: chatbox, from: Strophe.getBareJidFromJid($message.attr('from'))});
                converse.roster.addResource(contact_jid, resource);
        ...
        this.ChatBoxViews = Backbone.Overview.extend({
            ...
            _ensureElement: function () {
                ...
                if (!this.el) {
                    var $el = $('#conversejs');
                    if (!$el.length) {
                        $el = $('<div id="conversejs" class="hidden-print">');
                        $('body').append($el);
                    }
        ...
        this.MinimizedChatBoxView = Backbone.View.extend({
            ...
            render: function () {
                ...
                if (this.model.get('chatroom')) {
                    ...
                    data.title = this.model.get('fullname').replace(/@.*$/,'');
...
//                this.$roster = $('<center> <a class="invite-friends" '+
//                        'style="padding: 1px 20px; color:#cccccc; border: solid #cccccc 1px; box-shadow: inset 2px -2px 20px rgba(0, 0, 0, 0.3); color: #3B4D64;" '+
//                        'href="javascript:;"> Invite Friends</a></center> '+
//                        '<dl class="roster-contacts" style="display: none;"></dl>')
//                .on('click', function () {
//                    console.log('clicked on Invite Friends');
//                    $('.invite-menu-item').trigger('click');
//                    $('.close-chatbox-button').trigger('click');
//                });
            },
            ...
            reset: function () {
                ...
                this.removeAll();
//                this.$roster = $('<a class="s" style="padding: 10px; href="#"> Invite more friends</a> <dl class="roster-contacts" style="display: none;"></dl>');
                this.$roster = $('<dl class="roster-contacts" style="display: none;"></dl>');
                this.render().update();
                return this;
            },
...
//                    console.log(jids);
        ...
        'controlbox': {
            'toggle': function(e) {
                converse.controlboxtoggle.onClick(e);
            }
        },
        'chats': {
            'open': function (jids) {
        ...
        'archive': {
            'query': function (options, callback, errback) {
                ...
                if (typeof options != "undefined" && options.groupchat) {
                    if (!options['with']) {
                        throw new Error('You need to specify a "with" value containing the Group chat JID, when querying groupchat messages.');
                    }
